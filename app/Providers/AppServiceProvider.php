...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(\Illuminate\Http\Request $request)
    {
        ...
        if (!empty(env('NGROK_URL')) && $request->server->has('HTTP_X_ORIGINAL_HOST')) {
            $this->app['C:\UJITA-12JULI2021\MDUKARYAWAN']->forceRootUrl(env('NGROK_URL'));
        }
        ...
        view()->composer('*', function ($view) {
            ...
            if (Auth::check()) {
                ...
                $role_hak_akses = [
                    Pegawai::where('id', Auth::user()->id)->permission($hak_akses)->get(),
                    Akun::where('id', Auth::user()->id)->permission($hak_akses)->get()
                ];
